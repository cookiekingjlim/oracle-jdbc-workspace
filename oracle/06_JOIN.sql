 ...
 JOIN
    ...
    - 조회 결과는 하나의 결과물(RESULT SET)으로 나옴
      ...
      만약 어떤 사원이 어떤 직급ㅇ니지 직급명과 같이 조회하고 싶다면?
      
      => 즉, 관계형 데이터베이스에서 SQL문을 이용한 테이블 간에 "관계"를 맺어 원하는 데이터를 조회하는 방법
 
    - 크게 "오라클 구문"과 "ANSI 구문"
    - ANSI(미국 국립 표준 협회 == 산업 표준을 제정하는 단체): 다른 DBMS에서도 사용 가능
 */
 
 /*
 
    1. 오라클 - 등가 조인(EQUAL JOIN) / ANSI - 내부 조인(INNER JOIN / NATURAL JOIN)
        - 연결시키는 컬럼의 값이 일치하는 행들만 조인돼서 조회(일치하는 값이 없는 행은 조회 안 됨)
         ...
         2) ANSI 표준 구문 
            ...
            - JOIN 절에서 같이 조회하고자 하는 테이블을 기술 후 매칭시킬 컬럼에 대한 조건을 기술한다.(USING 구문과 ON 구문 사용)
            - 연결에 사용하려는 컬럼명이 같은 경우 USING(컬럼명)구문 사용
 */
 --1) 연결할 두 컬럼명이 다른 경우
 -- 사번, 사원명, 부서코드, 부서명을 같이 조회 -- EMPLOYEE에 DEPT_CODE랑 DEPARTMENT에 DEPT_ID 동일
 -->> 오라클 구문
...
-- 해결방법1: 테이블명을 이용하는 방법, 단점 테이블명이 길어지면 번거로워
...
-- 해결방법2: 테이블에 별칭을 부여해서 이용하는 방법
 ...
 -- 해결방법2: JOIN USING 구문 사용하는 방법(두 컬럼명이 일치할 때만)
 ...
 */
 ...
 매칭시킬 컬럼에 대한 조건 작성시 '='를 사용하지 않는 조언문
...
--종합 실습 문제--------------------------------------------------------------------------
--1. 직급이 대리 ASIA 지역에서 근무하는 직원들 사번, 직원명, 직급명, 부서명, 근무지역, 급여
-->ANSI
...
--2. 70년대생 이면서 여자, 성이 전 씨 직원명, 주민번호, 부서명, 직급명
SELECT EMP_NAME, EMP_NO, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE E, DEPARTMENT D, JOB J
WHERE D.DEPT_ID = E.DEPT_CODE
AND   J.JOB_CODE = E.JOB_CODE
AND   SUBSTR(EMP_NO, 1, 1) = 7 AND SUBSTR(EMP_NO, 8, 1)=2 AND SUBSTR(EMP_NAME, 1, 1) = '전';
...
--3. 보너스를 받는 직원들의 직원명, 보너스, 연봉, 부서명, 근무지역 조회 단, 부서코드가 없는 직원도 출력될 수 있게
 SELECT EMP_NAME, BOUNUS, SALARY*12, DEPT_TITLE, LOCAL_NAME
 FROM EMPLOYEE E
...
--5. 각 부서별 평균 급여 조회, 부서명, 평균급여(정수 처리) 조회 부서배치가 안된 사원 평균도 나오게
SELECT DEPT_TITLE, ROUND(AVG(NVL(SALARY,0)))
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
GROUP BY DEPT_TITLE;

--6. 각 부서별 총 급여 합의 1000만원이상인
 ...
 --7. 사번, 직원명, 직급명, 급여 등급, 구분을 조회하기. 이때, 구분에 해당하는 값은 아래와 같이 조회 되도록 한다.
 --     급여 등급이 S1, S2인 경우 '고급'
 --     급여 등급이 S3, S4인 경우 '중급'
 --     급여 등급이 S5, S6인 경우 '초급'
 -->>ANSI 구문
 ...
 --8. 보너스를 받지 ㅇ낳는 직원들 중 직급 코드가 J4 또는 J7인 직원들의 직원명, 직급명, 급여를 조회
 -->> ANSI 구문
