  /*
    ...
    - 관계형(Relational) 데이터베이스
        ...
        2. 데이터를 테이블 형태로 저장하고 여러 테이블을 조합하여 비즈니스 관례를 도출하는 구조
        3. 데이터의 중복을 최소화 할 수 있으며 사용하기 편리하고 데이터의 무결성, 트랜잭션 처리 등 데이터베이스 관리 시스템으로 뛰어난 성능을 보여준다.
        
    - SQL(Structured Query Language)
        : 관계형 데이터베이스에서 데이터를 조회하거나 조작하기 위해 사용하는 표준 검색 언어
        
    - SQL 종류
        1. DDL(Data Definition Language): 데이터 정의어 - DBMS 의 구조를 정의하거나 변경, 삭제하기 위해 사용하는 언어(CREATE 생성, ALTER 수정, DROP 삭제) -> 테이블
        ...
        4. DCM(Data Control Language): 데이터 제어어 - DBMS에 대한 보안, 무결성, 복구 등 DBMS를 제어하기 위한 언어(GRANT 권한 부여, REVOKE 권한 회수)
 ...
 */
 ...
 -- EMPLOYEE 테이블의 전체 사원들의 사번(EMP_ID), 이름(EMP_NAME), 급여(EMP_SALARY)만을 조회
 ...
 */
 --EMPLOYEE 테이블에서 직원명(EMP_NAME), 직원의 연봉( = 급여SALARY * 12) 조회
 ...
 --EMPLOYEE에 사원명, 입사일, 근무일수(오늘 날짜 - 입사일) 조회
 --오늘 날짜: SYSDATE
 ...
 컬럼 AS 별칭 / 컬럽 AS "별칭" / 컬럼 별칭/ 컬럼 "별칭"
    - 산술 연산을 하게되면 컬럼명이 지저분해진다. 이때 컬럼명에 별칭을 부여해서 깔끔하게 보여줄 수 있다.
    - 별칭을 부여할 때 띄어쓰기 혹은 특수문자가 별칭에 포함될 경우 반드시 큰따옴표("")로 감싸줘야 한다.
    
 */
 
 --EMPLOYEE 테이블에서 직원명, 연봉, 보너스가 포함된 연봉((급여 + (보너스*급여))*12) 조회
 SELECT EMP_NAME AS 직원명, SALARY * 12 AS "연봉", (SALARY + (BONUS * SALARY))*12 AS "총 소득(원)" FROM EMPLOYEE;
 ...
 */
 ...
 --EMPLOYEE 테이블에서 사번, 직원명, 급여 단위 조회
SELECT EMP_ID AS 사번 ,EMP_NAME AS 사원, SALARY AS 급여 FROM EMPLOYEE; 
SELECT EMP_ID AS 사번 ,EMP_NAME AS 사원, SALARY AS 급여,  '원' AS "단위(원)" FROM EMPLOYEE; 

/*
 연결 연산자: ||
    - 여러 컬럼값들을 마치 하나의 컬럼인 것처럼 연결하거나, 컬럼값과 리터를을 연결할 수 있음
*/
...
--DISTINCT 사용시 유의사항! SELECT절에 딱 한 번만 사용 가능
...
--입사일이 '90/01/01'~'01/01/01'인 사원 조회
...
WHERE HIRE_DATE BETWEEN '90/01/01' AND '01/01/01';
 ...
 LIKE
    ...
    '%': 0글자 이상
    비교 대상 컬럼 LIKE '문자%' => 비교 대상 컬럼값이 문자로 "시작"되는 걸 조회 
    비교 대상 컬럼 LIKE '%문자' => 비교 대상 컬럼값이 문자로 "끝"나는 걸 조회
    비교 대상 컬럼 LIKE '%문자%' => 비교 대상 컬럼값에 문자가 "포함"되는 걸 조회(키워드 검색)
    
    '_': 1글자 (언더바 하나가 한 글자라고 생각)
    비교 대상 컬럼 LIKE '_문자' => 비교 대상 컬럼값에 문자 앞에 무조건 한 글자가 올 경우 조회
    비교 대상 컬럼 LIKE '__문자' => 비교 대상 컬럼값에 문자 앞에 무조건 두 글자가 올 경우 조회
    비교 대상 컬럼 LIKE '_문자_' => 비교 대상 컬럼값에 문자 앞과 뒤에 무조건 한 글자씩 올 경우 조회
 ...
 */
 ...
 --WHERE NOT EMAIL LIKE '___$_%' ESCAPE '$' ; 
  WHERE EMAIL NOT LIKE '___$_%' ESCAPE '$' ; -- NOT위치 컬럼 앞 뒤
...
WHERE DEPT_CODE IS NULL AND BONUS IS NOT NULL;
 ...
 IN
    - 비교 대상 컬럼값이 내가 제시한 목록 중에 일치하는 값이 있는지
 ...
 */
 ...
 --연습문제----------------------------------------------------------
 --1. 사수가 없고 부서 배치도 받지 않은 사원들의 사원명, 사수사번, 부서코드 
 ...
 --5. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여, 보너스 포함 연봉(별칭) 조회
 SELECT EMP_ID, EMP_NAME, SALARY, (SALARY + (BONUS * SALARY))*12 AS 연봉
 FROM EMPLOYEE
 WHERE (SALARY + (BONUS * SALARY))*12 IS NOT NULL AND EMP_NAME LIKE '%하%';
 
 /*
 ORDERED BY 절
    - SELECT문 가장 마지막 줄에 작성뿐만 아니라 실행 순서 또한 마지막에 실행
    ...
    ORDER BY 정렬하고자 하는 컬럼값[ASC|DESC] [NULLS FIRST|NULLS LAST];
        ...
        - NULLS FIRST: 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 앞 배치(생략시 DESC일 때 기본값)
        - NULLS LAST: 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤 배치(생략시 ASC일 때 기본값)
...
ORDER BY BONUS DESC NULLS LAST; -- NULL을 맨 뒤로 배치 및 내림차순 정렬
