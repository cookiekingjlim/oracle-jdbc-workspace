 /*
    ...
    - 트랜잭션을 제어하는 언어
    - 데이터 베이스는 데이터의 변경 사항(INSERT, UPDATE, DELECT)들을 묶어서 하나의 트랜잭션에 담아서 처리
    ...
    SAVEPOINT
    - 저장점을 지정하고 ROLLBACK 진행 시 전체 작업을 ROLLBACK 하는 것이 아닌 SAVEPOINT 까지의 일부만 롤백한다.
 ...
 */
 ...
 -- EMP TEST 테이블에서 EMP_ID가 213, 218인 사원 삭제
 DELETE 
 FROM EMP_TEST
 ...
 -- DDL구문을 실행하는 순간, 기존 메모리 버퍼에 임시 저장된 변경사항들이 무조건 DB에 반영된다. --> DDL구문 실행하는 순간 강제 커밋된 것과 마찬가지
 CREATE TABLE TEST(
    TID NUMBER
 );
