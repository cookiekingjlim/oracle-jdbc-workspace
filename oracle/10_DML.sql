 /*
    ...
    - 기본값(DEFAULT)이 지정되어 잇으면 기본값이 들어감
 ...
 */
 ...
 INSERT INTO EMP_01
 VALUES(300, '유아지원팀', '공유', DEFAULT); --300	유아지원팀	공유	 23/06/28 --> 순서 중요
 ...
 -- EMP_NAME 컬럼에 NOT NULL 제약 조건에 의해서 에러 발생
 INSERT INTO EMP_01(EMP_ID, DEPT_TITLE)
 VALUES(700,'마케팅팀'); --> NAME은 NULL이 될 수 없으니까(NOT NULL 지정함)
 ...
 /*
    ...
    [표현법]
    1) INSERT ALL
       INTO 테이블명1[(컬럼, 컬럼,..., 컬럼)] VALUES (값, 값,....)
       INTO 테이블명2[(컬럼, 컬럼,..., 컬럼)] VALUES (값, 값,....)
       서브쿼리;
    2) INSERT ALL
        WHEN 조건1 THEN
 ...
 */
 ...
 ROLLBACK; --> ALTER와 다르게 ROLLBACK 가능
...
-- WHERE EMP_NAME = '노옹철';
 ...
 --2) 다중열 서브쿼리를 사용해서 SALARY, BONUS 컬럼들 한 번에 변경하기
 UPDATE EMP_SALARY
 ...
 --EMP_SALARY 테이블에서 아시아 직역에 근무하는 직원들의 보너스를 0.3으로 변경
 UPDATE EMP_SALARY
 SET BONUS = 0.3
 ...
 */
 ...
 -- DEPTARTMENT 테이블에서 DEPT_ID가 D1인 부서 삭제
 DELETE FROM DEPTARTMENT
 WHERE DEPT_ID = 'D1';
 ...
 */
 ...
 SELECT * FROM DEPT_COPY;
 SELECT * FROM EMP_SALARY; --> 사라진 데이터 돌아오지 않아,,,
