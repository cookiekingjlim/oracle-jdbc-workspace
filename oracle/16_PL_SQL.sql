 ...
 */
 ...
 /*
    1-1) 일반 타입 변수 선언 및 초기화
        [표현식]
        변수명 [CONSTANT] 자료형 [:= 값];      --CONSTANT가 변수 상수 구분
 */
 DECLARE
    EID NUMBER;
    ENAME VARCHAR2(20);
...
/
 ...
 -- 사용자가 입력한 사원의 사번(EID), 이름(ENAME), 부서명(DTITLE), 근무국가코드(NCODE)조회 후 각 변수에 대입 일반 타입 변수 TEAM을 데이터 타입 VARCHAR2(10)으로 선언하고
...
/
 ...
 /*
    1-3) IF ~ ELSEIF ~ ELSE 구문
        ...
        END IF;
 */   
 -- 사용자에게 점수를 입력받아 SCORE 변수(데이터타입: NUMBER)에 저장한 후 학점은 입력된 점수에 따라 GRADE 변수(데이터타입: CHAR)에 저장
 ...
 END;
 /
-- 사용자에게 입력받은 사원과 일치하는 사원의 급여 조회 후 출력(조회한 급여는 SAL 변수에 대입)
...
-- DEPT_CODE에 따라서 알맞는 부서(DNAME 데이터 타입: VARCHAR2(30))를 출력
-- D1인 경우 인사관리부, D2인 경우 회계관리부, D3인 경우 마케팅부, D4인 경우 국내영업부, D5인 경우 해외영업1부, D6인 경우 해외영업2부, D7인 경우 해외영업3부, D8인 경우 기술지원부, D9인 경우 총무부, 나머지는 부서 없음
-- 출력은 사번, 이름, 부서코드, 부서가 출력되도록 한다.
 ...
 */
 ...
 -- TOO_MANY_ROWS: SELECT한 결과가 여러 행인 경우(한 행이 리턴되어야 하는 상황에서)
 DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
 ...
 EXCEPTION 
    ...
    WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('에러가 발생했습니다. 관리자에 문의해주세요.');
 END;
 /
