 ...
 */
 ...
 -- 성별 (남자/여자) 사원의 수 조회
 SELECT DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남자', 2,'여자') AS "성별" , COUNT(*) AS "사원의 수"
 FROM EMPLOYEE
...
-- 부서별 평균 급여가 300만원 이상인 직원의 평균 급여를 조회(부서코드, 평균 급여)
 ...
 SELECT JOB_CODE, TO_CHAR(SUM(SALARY),'L999,999,999')
 FROM EMPLOYEE
 GROUP BY JOB_CODE
 ...
 */
 ...
 --컬럼이 하나일 때는 사실상 CUBE, ROLLUPD의 차이점이 딱히 없음!
 ...
 ORDER BY JOB_CODE,DEPT_CODE; --18행 부터 JOB_CODE 중간집계
 ...
 */
 ...
 /*
  ...
  UNION ALL(합집합): 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출(X중복되는 행 제거 안 함X)
