 ...
 */
 -- 관리자 계정으로 CREATE VIEW 권한을 부여해야함!
 ...
 */
 ...
 --SELECT문을 뷰(VW_EMP_JOB)로 정의
 CREATE OR REPLACE VIEW VW_EMP_JOB
 AS SELECT EMP_ID, EMP_NAME, JOB_NAME, DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남', 2,'여') AS "성별", EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE) AS "근무년수"
 FROM EMPLOYEE
 JOIN JOB USING (JOB_CODE);
 ...
 */
 ...
 /*
    ...
    2. 뷰에 포함되지 않은 커럼 중에 베이스가 되는 컬럼이 NOT NULL 제약 조건이 지정된 경우
 ...
 */
 ...
 -- INSERT
 INSERT INTO VW_JOB VALUES('J8','인턴'); -- 포함되지 않은 컬럼을 조작하려고 하면 에러 발생
 INSERT INTO VW_JOB VALUES('J8');
 
 -- UPDATE
 UPDATE VW_JOB SET JOB_NAME = '인턴' WHERE JOB_CODE = 'J7'; -- 뷰가 가지고 있지 않기에 에러 발생
 ...
 -- INSERT
 INSERT INTO VW_JOB VALUES('인턴'); -- 이렇게 되면 JOB CODE(NOT NULL지정됨)에 NULL이 됨 
 ...
 INSERT INTO VW_EMP_SAL VALUES(400, '홍길동', 3000000, 36000000);  -- 에러
 
 INSERT INTO VW_EMP_SAL(EMP_ID, EMP_NAME, SALARY) -- 연봉 제거했으나 에러!
 VALUES(400, '홍길동', 3000000); 
 
 ALTER TABLE EMPLOYEE MODIFY EMP_NO NULL; -- EMP_NO 수정!
 ...
 UPDATE VW_EMP_SAL
 SET SALARY = 7000000
 WHERE EMP_ID = 200; --성공!(산술 연산과 무관한 컬럼은 데이터 변경 가능)
 ...
 SELECT DEPT_CODE, SUM(SALARY) AS "급여 합계", FLOOR(AVG(NVL(SALARY,0))) AS "급여 평균"
 FROM EMPLOYEE
 GROUP BY DEPT_CODE;
 
 CREATE OR REPLACE VIEW VW_GROUPDEPT
 AS SELECT DEPT_CODE, SUM(SALARY) AS "급여 합계", FLOOR(AVG(NVL(SALARY,0))) AS "급여 평균"  -- 함수식이므로 별칭 필요
 FROM EMPLOYEE
 GROUP BY DEPT_CODE;
