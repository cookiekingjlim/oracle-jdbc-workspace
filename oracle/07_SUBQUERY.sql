 ...
 */
 ...
 2. 다중행 서브쿼리
    - 서브쿼리 조회 결과 값이 개수가 여러 행일 때 (여러행 한 열)
    IN 서브쿼리: 여러 개의 결과값 중에서 한 개라도 일치하는 값이 있다면
 
 */
  -- 각 부서별 최고 급여 조회 // 2890000,36600000,80000000,3760000,3900000,24900000,2550000
  SELECT MAX(SALARY)
  FROM EMPLOYEE
  GROUP BY DEPT_CODE; 
 ...
 */
 ...
 -->>2. 직급이 대리인 직원들 중에서 위의 값의 목록 중에 하나라도 큰 경우
 -->>오라클 구문(ANY는 표준 구문..그치만 잘 나와)
 SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
 FROM EMPLOYEE E, JOB J
 WHERE E.JOB_CODE = J.JOB_CODE
 AND J.JOB_NAME = '대리' AND SALARY > ANY(SELECT SALARY
                                         FROM EMPLOYEE E
                                         JOIN JOB J USING (JOB_CODE)
                                         WHERE J.JOB_NAME = '과장');
 ...
 */
 ...
 -->> 표준 구문
  ...
  FROM EMPLOYEE E, JOB J
  WHERE E.JOB_CODE = J.JOB_CODE
  AND J.JOB_NAME = '과장' AND SALARY > ALL(SELECT SALARY
                                         FROM EMPLOYEE E, JOB J
                                         WHERE E.JOB_CODE = J.JOB_CODE
                                         AND J.JOB_NAME = '차장');
 ...
 */
 ...
 --1. 하이유 사원의 부서 코드, 직급 코드
 SELECT DEPT_CODE, JOB_CODE
 FROM EMPLOYEE
 WHERE EMP_NAME = '하이유';
 
 --2. 부서코드가 D5이면서 직급 코드가 J5인 사원들 조회
 ...
 --4. 다중열 서브쿼리를 사용해서 작성하는 방법
 SELECT EMP_NAME, DEPT_CODE, JOB_CODE
 FROM EMPLOYEE
 WHERE (DEPT_CODE, JOB_CODE) = (('D5','J5')); --괄호 두 개 필요!
 ...
 */
 ...
 -- 부서별 평균 급여가 높은 3개 부서 코드, 평균 급여 조회
 -- ROWNUM 사용했더니 안돼 -> 별칭 사용해봐
 --1) 인라인 뷰를 사용하는 방법
 ...
 FROM (SELECT ROWNUM, DEPT_CODE, ROUND(AVG(NVL(SALARY,0)))
        FROM EMPLOYEE
        GROUP BY DEPT_CODE
        ORDER BY AVG(NVL(SALARY,0)) DESC)
 WHERE ROWNUM <= 3;
 */            
 ...
 FROM  (SELECT DEPT_CODE, ROUND(AVG(NVL(SALARY,0))) AS "평균급여"
        FROM EMPLOYEE
        GROUP BY DEPT_CODE
        ORDER BY AVG(NVL(SALARY,0)) DESC)
 WHERE ROWNUM <= 3;          
 ...
 /*
 RANK 함수
    - RANK() OVER(정렬 기준): 동일한 순위 이후의 등수를 동일한 인원수만큼 건너뛰고 순위를 계산한다.(ex. 공동 1위과 2명이면 다음 순위는 3위)
    - DENSE_RANK() OVER(정렬 기준): 동일한 순위 이후의 등수를 무조건 1씩 증가시킨다.(ex. 공동 1위가 2명이면 다음 순위는 2위)
 */
